...
- **باگ واقعی و اصلاح آن: بلر دکمه‌ی Scroll FAB اصلاً چیزی را نشان نمی‌داد (۲۰۲۶-۰۷-۰۷).**
  تلاش اول (کپی دقیق فرمول هدر Sticky — `blur(16px)` + Opacity ۰.۵۰/۰.۸۰) با
  `getComputedStyle` تأیید شد، اما کاربر با اسکرین‌شات واقعی نشان داد که دکمه در عمل
  یک دیسک کاملاً تو‌پُر است — هیچ ردی از متن/رنگِ پشتش دیده نمی‌شود. بررسی با
  `elementFromPoint` در کل بازه‌ی Scroll، در دو صفحه‌ی واقعی (Dashboard و Subscription
  Center)، **علت واقعی** را قطعی کرد: با `left: 20px`، کل جعبه‌ی ۴۸px دکمه همیشه داخل
  حاشیه‌ی بیرونی صفحه (`body { padding-inline: 36px }`) به‌علاوه‌ی Padding داخلیِ خودِ
  Panel می‌افتاد — یعنی هیچ‌وقت، در هیچ موقعیت Scrollی، واقعاً روی محتوای واقعی
  (متن/بج رنگی) قرار نمی‌گرفت؛ Blur کردنِ یک پس‌زمینه‌ی یک‌دست هم باز همان یک‌دست
  می‌ماند. **اصلاح موضع:** `left: 56px` (تأیید شده با همان روش که واقعاً روی محتوا
  می‌افتد). علاوه‌بر آن، حتی بعد از اصلاح موضع، کپی کردن مقادیر ۰.۵۰/۰.۸۰ از هدر
  اشتباه بود — کامنت خودِ هدر می‌گوید هدف آن «کاملاً مخفی کردن» ردیف پشت‌سرش است، نه
  «نمایش تار»؛ خواسته‌ی این دکمه دقیقاً عکس آن بود. با اسکرین‌شات‌های واقعی
  side-by-side (نه فقط `getComputedStyle`) مقادیر جدید تنظیم شد: **۰.۲۵/`blur(24px)`
  در Light Mode**، **۰.۴۰/`blur(24px)` در Dark Mode**. اسکرین‌شات نهایی (روی همان بج
  رنگی «VL» که در گزارش کاربر بود) این بار واقعاً حروف و رنگِ پشت دکمه را به‌صورت تار
  اما قابل‌تشخیص نشان می‌دهد — هم در Light هم Dark، هم دسکتاپ هم موبایل.
