...
> پروژه هر ۱۳ فاز Roadmap (`docs/blueprints/09-DEVELOPMENT_ROADMAP.md`) را کامل پشت سر گذاشته —
> با چند استثنای صریح و مستند که آگاهانه Blocked/حذف شده‌اند (بخش «محدودیت‌های شناخته‌شده»
...
> لگ با ۳۰۰۰+ نود، جدول‌های فشرده/Wrap‌شده، Sticky Header غیرشفاف، آیکون تم مشکی، فلش کوتاه
> داشبورد قبل از Splash، و چند مورد دیگر — یکی‌یکی بازتولید (نه حدس زده) و رفع شدند؛ فهرست کامل
> و صادقانه‌ی هر کدام (با Commit/تاریخ دقیق) در بخش «اصلاحات اخیر» پایین‌تر. **در این چک‌پوینت
> (آماده‌سازی برای ادغام به `main`)، هیچ مورد باز شناخته‌شده‌ای در سطح معماری یا رفتار باقی
> نمانده** — تنها موارد ذکرشده در «محدودیت‌های شناخته‌شده» پایین‌تر می‌مانند، که همگی تصمیم‌های
> Scope مستند و آگاهانه‌اند، نه کار ناتمام.

## وضعیت کلی

**Phase 0 تا Phase 12 کامل‌اند، به‌علاوه‌ی بازطراحی بصری/i18n و یک سری بلندبالای فیکس واقعی
بعد از آن (بخش «اصلاحات اخیر» پایین‌تر) — جزئیات زیر مستند و آگاهانه‌اند (نه نقص، نه فراموشی):**
- `riskScore` و `compatibilityScore` (پیش‌نیازش) هر دو فرمول‌بندی و پیاده‌سازی شدند (ADR-027) —
  جزئیات کامل در بند پایین.
...
۱. در صفحه‌ی ریپو (`operaboys/UNCT`) روی دکمه‌ی سبز **Code** بزنید → **Download ZIP** (برنچ
   پیش‌فرض `main`، همین کد).
...
- **دو باگ بصری هدر موبایل — رفع بعد از تست کاربر واقعی (۲۰۲۶-۰۷-۱۰).**
  ...
  2. **آیکون خورشید در تم روشن مشکی بود، نه طلایی** (جایگزین‌شده — ر.ک. ورودی ریشه‌کنی
     ۲۰۲۶-۰۷-۱۰ پایین‌تر: این فیکس یونیکدی کافی نبود، ریشه‌ی واقعی یک تناقض Specificity در
     CSS بود، و کل گلیف بعداً با یک SVG درون‌خطی جایگزین شد): کاراکترهای ☀ (U+2600) و ☾
     (U+263E) طبق یونیکد پیش‌فرض Text-Presentation دارند، ولی خیلی از Font Fallback های
     اندروید همچنان آن‌ها را به‌صورت Emoji رنگی رندر می‌کنند و رنگ CSS (`#E8C56A`) را کلاً
...
- **Sheet «بیشتر» موبایل: عدم تکرار آیتم‌های Dock (۲۰۲۶-۰۷-۱۱).** Sheet روی کل ۸ آیتم
  `NAV_ITEMS` map می‌کرد، در حالی که ۴ تای آن‌ها (Dashboard/Converter/Subscription/Export)
  همان لحظه در Dock هم در دسترس بودند — تکرار غیرضروری. اکنون `SHEET_ITEMS` در
  `ui/components/nav.tsx` به‌صورت مشتق (`NAV_ITEMS` منهای `DOCK_ITEMS`، بر اساس `key`) محاسبه
  می‌شود، نه یک لیست جدا نگه‌داری‌شده، پس اگر ترکیب Dock در آینده عوض شود Sheet خودکار
  هماهنگ می‌ماند. کلید عنوان Sheet هم از `nav.allSections` (که دیگر درست نبود — Sheet دیگر
  «همه»‌ی بخش‌ها را نشان نمی‌دهد) به `nav.otherSections` تغییر نام یافت:
  en: "Other sections" / fa: "بخش‌های دیگر" (هر دو مصرف‌کننده‌ی این کلید — عنوان Sheet و
  aria-label بک‌دراپش — هماهنگ به‌روزرسانی شدند). Scope-check: تنها همین یک مصرف‌کننده در کل
  پروژه داشت. **تست**: ۲ تست e2e جدید (en/fa) که واقعاً Sheet را باز می‌کنند و تأیید می‌کنند
  دقیقاً همان ۴ بخش غیر-Dock را نشان می‌دهد و هیچ‌کدام از ۴ بخش Dock را تکرار نمی‌کند؛
  اسکرین‌شات واقعی موبایل از Sheet باز در هر دو زبان. `typecheck`، ۱۲۷۷ تست واحد، `build`،
  و هر ۶۶ تست e2e سبز.

- **لوگوی هدر: بزرگ‌تر فقط در موبایل (۲۰۲۶-۰۷-۱۱).** `.app-nav__brand img` قبلاً یک مقدار
  ثابت (`58px`) داشت بدون Override موبایلی. سه مقدار کاندید (۶۸/۷۴/۸۰px) با اسکرین‌شات واقعی
  موبایل (هر دو تم + فارسی برای RTL) در برابر کارت hero داشبورد زیرش مقایسه شدند؛ هر سه بدون
  همپوشانی بودند، پس **۸۰px** — بزرگ‌ترین کاندید — به `@media (max-width: 760px)` اضافه شد.
  دسکتاپ کاملاً دست‌نخورده ماند (تأیید مستقیم: `getComputedStyle` روی ویوپورت ۱۲۸۰px همان
  `58px` قبلی را برگرداند). **تست**: `typecheck` تمیز (بدون تست e2e جدید — این یک مقدار CSS
  تکی بدون منطق جدید است؛ طبق `proportional-effort` پوشش موجود Dock/Sheet/RTL کافی است).

- **باگ لود اولیه: فلش کوتاه داشبورد قبل از Splash (۲۰۲۶-۰۷-۱۱).** ریشه (کاربر تأیید کرده
  بود): `var(--unct-*)` فقط زیر `:root[data-theme="dark"|"light"]` تعریف شده بودند، ولی آن
  Attribute در `useEffect` (`ui/main.tsx`) ست می‌شد که بعد از اولین Mount/Paint اجرا می‌شود —
  همان فریم اول، Attribute غایب بود، پس `var(--unct-bg)` نامعتبر بود و پس‌زمینه‌ی `.splash`
  (که خودش هیچ Fallback نداشت) شفاف می‌شد و داشبورد از پشتش دیده می‌شد.
  - **لایه‌ی ۱ (ریشه‌ای)**: یک `<script>` inline کوچک و همگام در `<head>` صفحه‌ی `index.html`،
    قبل از بارگذاری `assets/js/app.js`، که همان منطق `resolveTheme()`/`isThemeChoice()`/
    `STORAGE_KEY` را از `core/store/settings-state.js` عیناً تکرار می‌کند (کامنت متقابل بین دو
    فایل ثبت شد تا اگر یکی تغییر کرد، دیگری هم به‌روزرسانی شود) و `data-theme` را قبل از هر
    Paint ست می‌کند. `useEffect` موجود دست‌نخورده ماند — برای Sync زنده‌ی حالت "auto" با تغییر
    OS بعد از لود اولیه هنوز لازم است.
  - **لایه‌ی ۲ (کمربند ایمنی)**: تمام مقادیر تم Dark (پیش‌فرض طراحی) در `assets/css/theme.css`
    اکنون در `:root` بدون قید هم تکرار شده‌اند (کامنت توضیح می‌دهد این فقط Fallback است؛
    `:root[data-theme="light"]` با Specificity بالاتر همیشه برنده می‌ماند وقتی Attribute واقعاً
    حاضر باشد) — پس حتی اگر روزی یک مسیر دیگر بدون ست‌کردن Attribute اجرا شود (یا JS کلاً
    غیرفعال باشد)، هیچ `var()`ی هرگز نامعتبر نمی‌شود.
  - **تأیید**: با CPU Throttling سنگین (۶×) + اسکرین‌شات پشت‌سرهم فریم‌های اول (۰ تا ۲۰۰۰
    میلی‌ثانیه بعد از Navigation) روی هر دو تم ذخیره‌شده × موبایل/دسکتاپ — همیشه از همان فریم
    اول Splash با پس‌زمینه‌ی درست‌رنگ دیده شد، هرگز داشبورد از پشتش. `typecheck` تمیز؛ هیچ
    تست e2e موجودی منطق تم/Splash را نمی‌سنجید (بررسی شد)، پس طبق `proportional-effort`
    تست e2e جدیدی افزوده نشد — این همان تأیید بصری دقیق‌تر و قوی‌تری بود که برای این نوع
    Race Condition لازم است.

- **تست دائمی مسیر واقعی Export → Format Detection → Parse (۲۰۲۶-۰۷-۱۱).** تست‌های
  Round-Trip موجود (`to-clash.test.js` و مشابه‌ها) از Parser اختصاصی هر فرمت رد می‌شدند،
  نه از مسیر واقعی Format Detection که کاربر موقع Paste کردن در Converter از آن عبور
  می‌کند. فایل جدید `tests/converter/export-import-roundtrip.test.js` این مسیر واقعی را
  می‌سنجد: خروجی *کامل* Exporter واقعی (`exportTxt`/`exportClashYaml`/`exportSingboxJson`/
  `exportXrayJson`) → `parseAndValidate` (Detection واقعی، نه Parser مستقیم) — برای هر ۷
  پروتکل، شامل آدرس IPv6 بدون‌براکت، remark فارسی/ایموجی، و پسورد با کاراکترهای خاص
  percent-encode-worthy. Skipهای مستند (hysteria2/tuic/wireguard در Xray) با
  `*_SUPPORTED_PROTOCOLS` هر فرمت تشخیص و به‌عنوان Skip (نه Fail) Assert می‌شوند. همه‌ی
  ۲۸ حالت (۲۵ Round-trip واقعی + ۳ Skip مستند) از همان اولین اجرا سبز شدند — هیچ باگی
  پیدا نشد. بدون تغییر کد Production.

- **آماده‌سازی برای ادغام به `main` (۲۰۲۶-۰۷-۱۱).** ممیزی کامل README طبق چک‌لیست
  `readme-audit` انجام شد: بازنویسی مقدمه (که فریم قدیمی «riskScore آخرین مورد باز بود»
  را حمل می‌کرد، در حالی که ~۷۰۰ خط تاریخچه‌ی واقعی بعد از آن رخ داده بود)، به‌روزرسانی
  اعداد تست/دیکشنری کهنه (۱۱۸۱→۱۳۰۵ تست، ۹۶→۱۰۵ فایل، ۳۴۱→۳۶۷ کلید دیکشنری، افزودن شمار
  e2e که قبلاً اصلاً در خلاصه‌ی بالا نبود)، حذف دستورالعمل Download ZIP مخصوص برنچ فیچر
  (که بعد از Merge به `main` نادرست می‌شد)، و افزودن یادداشت «جایگزین‌شده» به ورودی فیکس
  U+FE0E که کاملاً با فیکس SVG بعدی جایگزین شد. **تأیید کامل و Fresh** (نه از حافظه):
  `tsc --noEmit` بدون خطا؛ کل Suite واحد (`npx vitest run`) — ۱۳۰۵ تست در ۱۰۵ فایل، همه
