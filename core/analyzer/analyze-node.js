 ...
 *
 * What it returns (and deliberately does NOT): every §1/§2.6 module together
 * can fill four of `AnalysisObject`'s frozen fields today — `securityScore`
 * (06 §1.2 / ADR-011), `compatibilityScore` and `riskScore` (06 §3.1 /
 * ADR-027, computed by `risk-score.js` from `security`/`compatibility`/`dns`
 * below — see that module for why Reality is deliberately NOT a fourth,
 * separate input), and the honestly-derivable `realityDetected`
 * (security === "reality"); the DNS Analyzer (ADR-022) fills
 * `AnalysisObject.dnsLeakRisk` directly. The remaining `AnalysisObject`
 * fields (`cloudflareDetected`, `workerDetected`, `cleanIPDetected`) come from
 * the rest of §2's semi-definitive modules and the Final Report aggregation
 * (doc 06 §4, Summary/Warnings/Recommendations/Suggestions) — a future,
 * still-entirely-unbuilt phase — so this module returns the analyzers' raw
 * verdict bundle plus the two ADR-027 scores rather than fabricating a full
 * `AnalysisObject` (Rule 9: never invent data). ADR-011 §"Explicitly out of
 * scope" is explicit that `dnsLeakRisk` must never be folded into
 * `securityScore` — it stays its own independent bundle field, exactly like
 * `cloudflare`/`cleanIp`/`worker`.
 *
...
import { computeCompatibilityScore, computeRiskScore } from "./risk-score.js";
...
export function analyzeNode(node) {
  ...
  const riskScore = computeRiskScore({ securityScore: security.securityScore, compatibilityScore, dnsLeakRisk: dns });
  return {
    completeness, protocol, network, tls, reality, security, compatibility, cloudflare, cleanIp, worker, rules, dns,
    compatibilityScore, riskScore,
  };
}
