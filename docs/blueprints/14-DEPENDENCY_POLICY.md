...
| QRCode Generator | تولید QR Code | تأییدشده (نسخه‌ی قبلی) |
| ZIP Utility | ساخت/خواندن فایل ZIP | تأییدشده (نسخه‌ی قبلی) |
...
| سقف | مقدار |
|---|---|
| UI Layer (Preact + Componentها) | ≤ 50KB (gzip) |
| کل Dependencyهای خارجی (Preact + YAML + ZIP + QR + Virtual List + ...) | ≤ 150KB (gzip) |

**قانون:** قبل از هر Release، اندازه‌ی Bundle نهایی باید اندازه‌گیری شود (مثلاً با `source-map-explorer` یا ابزار مشابه در زمان توسعه، نه به‌عنوان Runtime Dependency). عبور از سقف بدون تأیید معماری مجاز نیست.
